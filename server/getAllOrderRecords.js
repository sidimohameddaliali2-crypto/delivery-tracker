...
async function getAllOrderRecords() {
  ...
  try {
    console.log('Fetching ALL records from Order: Web Data layout...\n');

    const response = await axios.get(
      `${baseURL}/fmi/data/vLatest/databases/${database}/layouts/Order%3A%20Web%20Data/records?_limit=100`,
      {
        headers: {
          'Content-Type': 'application/json',
    ...
    if (records.length > 0) {
      console.log('=== FIELD NAMES IN ORDER: WEB DATA LAYOUT ===');
      ...
      console.log('No records in Order: Web Data layout');
    }
