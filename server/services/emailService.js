...
export const sendNewMemberOrderEmail = async ({ member, partner, order, lines, totalAmount }) => {
  if (!NOTIFY_EMAIL || !BREVO_API_KEY) {
    console.warn('[email] Skipped — BREVO_API_KEY or ORDER_NOTIFY_EMAIL not set');
    return;
  }
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;color:#1a1a1a">
      <div style="background:#051747;padding:24px 32px;border-radius:12px 12px 0 0">
        <h1 style="color:#fff;margin:0;font-size:20px">New Member Order</h1>
        <p style="color:#a8ccf5;margin:4px 0 0;font-size:13px">Matter Partner — Member Portal</p>
      </div>
      <div style="background:#fff;padding:28px 32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px">
        <table style="width:100%;border-collapse:collapse;margin-bottom:20px">
          <tr>
            <td style="padding:6px 0;color:#6b7280;font-size:13px;width:140px">Member</td>
            <td style="padding:6px 0;font-weight:600">${member.name} &bull; ${member.email}</td>
          </tr>
          <tr>
            <td style="padding:6px 0;color:#6b7280;font-size:13px">Partner</td>
            <td style="padding:6px 0;font-weight:600">${partner.businessName}</td>
          </tr>
          ${member.dietaryExclusions ? `
          <tr>
            <td style="padding:6px 0;color:#6b7280;font-size:13px">Exclusions</td>
            <td style="padding:6px 0">${member.dietaryExclusions}</td>
          </tr>` : ''}
          <tr>
            <td style="padding:6px 0;color:#6b7280;font-size:13px">Delivery date</td>
            <td style="padding:6px 0;font-weight:600">${deliveryDate}</td>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          ...
          <tfoot>
            <tr style="background:#f9fafb">
              <td colspan="3" style="padding:10px 12px;font-weight:700;text-align:right">Grand Total</td>
              <td style="padding:10px 12px;font-weight:700;text-align:right;color:#051747">AED ${Number(totalAmount).toFixed(2)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
  ...
  const res = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({
      sender: { name: SENDER_NAME, email: SENDER_EMAIL },
      to: [{ email: NOTIFY_EMAIL }],
      subject: `New Member Order — ${member.name} @ ${partner.businessName} (${deliveryDate})`,
      htmlContent: html,
    }),
  });
